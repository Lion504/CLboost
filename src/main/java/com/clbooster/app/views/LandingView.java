...
public class LandingView extends VerticalLayout {
    ...
    public LandingView() {
        ...
        Button signupBtn = new Button("Sign up", e -> getUI().ifPresent(ui -> ui.navigate(SignUpView.class)));
        signupBtn.addThemeVariants(ButtonVariant.LUMO_PRIMARY);

        HorizontalLayout navbar = new HorizontalLayout(logo, howItWorks, faq, loginBtn, signupBtn);
        navbar.setWidthFull();
        navbar.setAlignItems(FlexComponent.Alignment.CENTER);
        navbar.expand(logo);
        navbar.getStyle().set("padding", "16px 48px").set("border-bottom", "1px solid #e5e7eb").set("background",
                "white");
        ...
        headline.getStyle().set("font-size", "clamp(2rem, 5vw, 3.5rem)").set("line-height", "1.1").set("margin", "0");

        Paragraph heroSub = new Paragraph("Generate perfectly tailored cover letters in seconds by "
                + "analyzing your resume, the job ad, and company culture.");
        heroSub.getStyle().set("color", "#6b7280").set("font-size", "16px").set("max-width", "480px");

        Button generateNow = new Button("Generate Now", e -> getUI().ifPresent(ui -> ui.navigate(SignUpView.class)));
        ...
        heroImage.getStyle().set("width", "50%").set("min-height", "380px").set("background", "#f1f5f9")
        ...
        HorizontalLayout featCards = new HorizontalLayout(
                featureCard("Instant Generation", "Analyze complex job ads and extract key keywords automatically."),
        ...
        features.getStyle().set("background", "var(--lumo-contrast-5pct)").set("padding", "60px 80px");
